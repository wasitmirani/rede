...
<div class="flex justify-between items-baseline uk-visible@s">
    <h1 class="font-extrabold leading-none mb-6 mt-8 lg:text-2xl text-lg text-gray-900 tracking-tight"> Crew Suggestion</h1>
</div>
        ...
        <ul class="uk-slider-items uk-child-width-1-2 uk-child-width-1-3@s uk-child-width-1-4@m uk-grid-small" style="transform: translate3d(0px, 0px, 0px);">
            ...
            <li tabindex="-1" class="uk-active" >
                <div class="bg-gray-200 max-w-full lg:h-64 h-52 rounded-lg relative overflow-hidden" >
                    <a uk-toggle=""  href="#story-modal{{ $crew->userData->id }}">
                       @if($crew->userData->image != null)
                        <img src="{{ asset('user/images/'.$crew->userData->image) }}" class="w-full h-full absolute object-cover inset-0">
                        @else
                    ...
                    <div class="absolute bottom-0 p-4 w-full custom-overly1">
                        <div class="flex justify-between align-bottom flex-wrap text-white">
                            <div class="w-full truncate text-lg"> {{ $crew->userData->name }} </div>
                            <div class="leading-5 text-sm">
                                {{-- <div> 21, Turkey </div> --}}

                            </div>
                            <a href="{{ route('public.profile',['id'=>$crew->userData->id,'name'=>$crew->userData->name]) }}" id="followBtn{{ $crew->id }}" class=" right-3 bottom-3 rounded-full bg-gradient-to-tr from-blue-500 to-purple-700 followBtn" data-id="{{ $crew->id }}" data-follower="{{ Auth::user()->id }}">
                                Visit Profile
                            </a>
            ...
            <div id="story-modal{{ $crew->userData->id }}" class="uk-modal-container uk-modal" uk-modal="">
                <div class="uk-modal-dialog story-modal">
                    <button class="uk-modal-close-default lg:-mt-9 lg:-mr-9 -mt-5 -mr-5 shadow-lg bg-white rounded-full p-4 transition dark:bg-gray-600 dark:text-white uk-icon uk-close" type="button" uk-close=""><svg width="14" height="14" viewBox="0 0 14 14" xmlns="http://www.w3.org/2000/svg" data-svg="close-icon"><line fill="none" stroke="#000" stroke-width="1.1" x1="1" y1="1" x2="13" y2="13"></line><line fill="none" stroke="#000" stroke-width="1.1" x1="13" y1="1" x2="1" y2="13"></line></svg></button>
                        ...
                        <div class="story-modal-media">
                            <img src="{{ asset('user/images/'.$crew->userData->image) }}" alt="" class="inset-0 h-full w-full object-cover">
                        </div>
                        <div class="flex-1 bg-white dark:bg-gray-900 dark:text-gray-100">
                            ...
                            <div class="border-b flex items-center justify-between px-5 py-3 dark:border-gray-600">
                                <div class="flex flex-1 items-center space-x-4">
                                    ...
                                    <span class="block text-lg font-semibold"> {{ $crew->userData->name }} </span>
                                </div>
                                <a href="#">
...
<div class="flex justify-between items-baseline uk-visible@s">
    ...
    <h1 class="font-extrabold leading-none mb-6 mt-8 lg:text-2xl text-lg text-gray-900 tracking-tight"> Crew Requests</h1>

</div>
<div class="relative uk-slider" uk-slider="finite: true">
    <div class="uk-slider-container pb-3 -ml-3">
        <ul class="uk-slider-items uk-child-width-1-2 uk-child-width-1-3@s uk-child-width-1-4@m uk-grid-small" style="transform: translate3d(0px, 0px, 0px);">
            @forelse($follow_reqs as $request)
            <li tabindex="-1" class="uk-active" >
                <div class="bg-gray-200 max-w-full lg:h-64 h-52 rounded-lg relative overflow-hidden" >
                    <a uk-toggle=""  href="#story-modal{{ $request->userData->id }}">
                       @if($crew->userData->image != null)
                        <img src="{{ asset('user/images/'.$request->userData->image) }}" class="w-full h-full absolute object-cover inset-0">
                        @else
                        <img src="{{asset('/assets/images/dummyuser.jpg')}}" class="w-full h-full absolute object-cover inset-0">
                        @endif
                    </a>
                    <div class="absolute bottom-0 p-4 w-full custom-overly1">
                        <div class="flex justify-between align-bottom flex-wrap text-white">
                            <div class="w-full truncate text-lg"> {{ $request->userData->name }} </div>
                            <div class="leading-5 text-sm">
                                {{-- <div> 21, Turkey </div> --}}
                            </div>
                            <a href="{{ route('public.profile',['id'=>$request->userData->id,'name'=>$request->userData->name]) }}" id="followBtn{{ $request->userData->id }}" class=" right-3 bottom-3 rounded-full bg-gradient-to-tr from-blue-500 to-purple-700 followBtn" data-id="{{ $request->userData->id }}" data-follower="{{ Auth::user()->id }}">
                                Visit Profile
                            </a>
                            <a href="{{ route('accept.request',$request->userData->id) }}" id="followBtn{{ $request->userData->id }}" class=" right-3 bottom-3 rounded-full bg-gradient-to-tr from-blue-500 to-purple-700 followBtn" data-id="{{ $request->userData->id }}" data-follower="{{ Auth::user()->id }}">
                               Accept Request
                            </a>
                        </div>
                    </div>
...
<h1 class="font-extrabold leading-none mb-6 mt-8 lg:text-2xl text-lg text-gray-900 tracking-tight">  Crew List </h1>
...
@forelse($followers as $follower)
    <div>
        <div class="bg-red-400 max-w-full lg:h-56 h-48 rounded-lg relative overflow-hidden shadow uk-transition-toggle">
            <a href="{{ route('public.profile',['id'=>$follower->id,'name'=>$follower->name]) }}" >
                <img src="{{ asset('user/images/'.$follower->image) }}" class="w-full h-full absolute object-cover inset-0 scale-150 transform">
            </a>
            <div class="flex flex-1 items-center absolute bottom-0 w-full p-3 text-white custom-overly1 uk-transition-slide-bottom-medium">
                <a href="{{ route('public.profile',['id'=>$follower->id,'name'=>$follower->name]) }}" class="lg:flex flex-1 items-center hidden">
                    <div class="w-full truncate text-lg"> {{ $follower->name }} </div>
                </a>
            </div>
        </div>
    </div>
@empty
<h3>Not Follower Fond</h3>
@endforelse
</div>
